...
export const usePermissionStore = defineStore("app-permission", () => {
  ...
  const getPermCodeList = computed(() => permCodeList.value)
  ...
  function setPermCodeList(codeList: string[] | number[]) {
  ...
  async function changePermissionCode() {
    //TODO get code list from API
    const codeList = [] as string[] | number[];
  ...
  async function buildRoutesAction(): Promise<AppRouteRecordRaw[]> {
    ...
    const routeFilter = (route: AppRouteRecordRaw) => {
      ...
      // 进行角色权限判断
      return roleList.some((role: RoleEnum) => roles.includes(role));
    };
