...
export const TableImg = defineComponent({
  name: 'TableImg',
  props: {
    imgList: {
    ...
    // 是否简单显示（只显示第一张图片）
    simpleShow: {type: Boolean, default: false},
    // 简单模式下是否显示图片数量的badge
  ...
  setup(props,) {
    ...
    const count = computed(() => {
      const {imgList, showBadge} = props
      return !showBadge || imgList?.length === 1 ? 0 : imgList?.length
    })

    function renderSimpleImgGroup() {
      const {imgList, size, fallback} = props
    ...
    function renderImgGroup() {
      const {imgList, size, fallback, margin} = props
      ...
      <>{
        unref(show) && (
          <div style={unref(getWrapStyle)}>
            {
              props.simpleShow
                ? <Badge count={unref(count)}>
                  <div>{renderSimpleImgGroup()}</div>
                </Badge>
                : renderImgGroup()
            }
          </div>
